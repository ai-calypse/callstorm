...
3. A step with harness_degraded true measured the test machine, not the agent: draw no conclusion from it, and say so where it matters. A p95 from fewer than 30 calls is fragile.
...
5. Order insights by what a team should act on first, at most 8. next_step is one concrete action with no invented numbers.
